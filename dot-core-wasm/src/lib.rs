// ABOUTME: WASM-compatible DOT parser for the web editor.
// ABOUTME: Mirrors the dot-core parser API but uses wasm-bindgen instead of UniFFI.

use serde::Serialize;
use wasm_bindgen::prelude::*;

// ---- Types (same as dot-core/src/parser.rs) ----
...
fn parse_dot(source: &str) -> DotGraph {
...
fn statement_at_internal(graph: &DotGraph, offset: u32) -> Option<&DotStatement> {
...
fn node_id_at_internal(statement: &DotStatement, offset: u32) -> Option<String> {
    match statement {
...
fn definition_for_node_internal<'a>(graph: &'a DotGraph, node_id: &str) -> Option<&'a DotStatement> {
...
pub fn node_id_at_offset_wasm(source: &str, offset: u32) -> Option<String> {
    let graph = parse_dot(source);
    let stmt = statement_at_internal(&graph, offset)?;
    node_id_at_internal(stmt, offset)
}
...
pub fn definition_offset_for_node_wasm(source: &str, node_id: &str) -> Option<u32> {
    let graph = parse_dot(source);
    let stmt = definition_for_node_internal(&graph, node_id)?;
...
mod tests {
    ...
    fn test_simple_digraph_parses_nodes() {
        ...
        let node_ids: Vec<&str> = graph
            ...
            .iter()
            .filter_map(|s| {
                if let DotStatement::NodeDefinition { id, .. } = s {
                    Some(id.as_str())
                } else {
                    None
                }
    ...
    fn test_simple_edge() {
        ...
        let edges: Vec<(&str, &str)> = graph
            ...
            .iter()
            .filter_map(|s| {
                if let DotStatement::Edge { from, to, .. } = s {
    ...
    fn test_graph_attribute_statement() {
        let graph = parse_dot("digraph G {\n    graph [rankdir=LR]\n    A -> B\n}");
        let has_graph_attr = graph
            .statements
            .iter()
            .any(|s| matches!(s, DotStatement::GraphAttribute { .. }));
    ...
    fn test_line_comment_ignored() {
        ...
        let edges: Vec<_> = graph
            .statements
            .iter()
            .filter(|s| matches!(s, DotStatement::Edge { .. }))
            .collect();
        assert_eq!(edges.len(), 1);
    }
    ...
    fn test_statement_at_offset_finds_correct_statement() {
        ...
        let stmt = statement_at_internal(&graph, b_offset);
        assert!(stmt.is_some());
        match stmt.unwrap() {
    ...
    fn test_statement_at_offset_returns_none_outside_statements() {
        let dot = "digraph G {\n\n\n    A\n}";
        let graph = parse_dot(dot);
        let stmt = statement_at_internal(&graph, 13);
        assert!(stmt.is_none());
    }
    ...
    fn test_node_id_at_offset_for_node_definition() {
        ...
        let stmt = statement_at_internal(&graph, a_offset);
        assert!(stmt.is_some());
        assert_eq!(
            node_id_at_internal(stmt.unwrap(), a_offset),
            Some("A".to_string())
        );
    }
    ...
    fn test_node_id_at_offset_in_attribute_area() {
        ...
        let stmt = statement_at_internal(&graph, label_offset);
        assert!(stmt.is_some());
        assert_eq!(
            node_id_at_internal(stmt.unwrap(), label_offset),
            Some("A".to_string())
        );
    }
    ...
    fn test_node_id_at_offset_for_edge_selects_closer_node() {
        ...
        let stmt_a = statement_at_internal(&graph, a_offset).unwrap();
        let stmt_b = statement_at_internal(&graph, b_offset).unwrap();
        assert_eq!(
            node_id_at_internal(stmt_a, a_offset),
            Some("A".to_string())
        );
        assert_eq!(
            node_id_at_internal(stmt_b, b_offset),
            Some("B".to_string())
        );
    }
    ...
    fn test_definition_for_node_finds_node_definition() {
        let dot = "digraph G {\n    A [label=\"Hello\"]\n    A -> B\n}";
        let graph = parse_dot(dot);
        let stmt = definition_for_node_internal(&graph, "A");
        assert!(stmt.is_some());
        match stmt.unwrap() {
    ...
    fn test_definition_for_node_falls_back_to_edge() {
        let dot = "digraph G {\n    A -> B\n}";
        let graph = parse_dot(dot);
        let stmt = definition_for_node_internal(&graph, "B");
        assert!(stmt.is_some());
        match stmt.unwrap() {
    ...
    fn test_definition_for_node_returns_none_for_unknown() {
        let dot = "digraph G {\n    A -> B\n}";
        let graph = parse_dot(dot);
        assert!(definition_for_node_internal(&graph, "Z").is_none());
    }
    ...
    fn test_graph_attribute_returns_none_node_id() {
        ...
        let graph = parse_dot(dot);
        let graph_attr = graph
            .statements
            .iter()
            .find(|s| matches!(s, DotStatement::GraphAttribute { .. }));
        assert!(graph_attr.is_some());
        let attr = graph_attr.unwrap();
        let range = attr.source_range();
        assert!(node_id_at_internal(attr, range.location).is_none());
    }
}
